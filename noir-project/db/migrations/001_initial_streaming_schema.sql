...
CREATE TABLE permissions (id bigserial PRIMARY KEY, code text UNIQUE NOT NULL, description text);
CREATE TABLE role_permissions (role text NOT NULL, permission_id bigint REFERENCES permissions(id) ON DELETE CASCADE, PRIMARY KEY(role, permission_id));
CREATE TABLE ratings (profile_id uuid REFERENCES profiles(id) ON DELETE CASCADE, title_id uuid REFERENCES titles(id) ON DELETE CASCADE, rating int NOT NULL CHECK(rating BETWEEN 1 AND 10), updated_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(profile_id, title_id));
CREATE TABLE reactions (profile_id uuid REFERENCES profiles(id) ON DELETE CASCADE, entity_type text NOT NULL CHECK(entity_type IN ('title','review','comment')), entity_id uuid NOT NULL, reaction text NOT NULL CHECK(reaction IN ('like','dislike')), created_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(profile_id, entity_type, entity_id));
CREATE TABLE profile_settings (profile_id uuid PRIMARY KEY REFERENCES profiles(id) ON DELETE CASCADE, autoplay boolean NOT NULL DEFAULT true, subtitles text, audio_language text DEFAULT 'ru', recommendation_flags jsonb NOT NULL DEFAULT '{}');
CREATE TABLE content_reports (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), reporter_profile_id uuid REFERENCES profiles(id) ON DELETE SET NULL, entity_type text NOT NULL, entity_id uuid NOT NULL, reason text NOT NULL, status text NOT NULL DEFAULT 'open', created_at timestamptz NOT NULL DEFAULT now());
